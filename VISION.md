...
**Product promise:** *everything you want to know about your daily movement, nothing you don't need.* The phone is enough; no wearable, no account, and — stronger than tweather can claim — **no network**. The app does not request the INTERNET permission at all. Data never leaves the device except when the user explicitly exports it. (Since Fase 24 the phone being enough includes one of its system components: the hours the app cannot sample itself are recorded on device by Play services. Still no network, still no account — see §7.)
...
- **Reboot handling**: the counter resets at boot; persisted anchors (boot count + last cumulative value) reconstruct continuity. This is the classic pedometer bug — it gets its own tests. Two halves, learned the hard way (Fase 24a): the delta of a restarted counter is the cumulative value, *and* its span starts no earlier than the boot instant. Without the second half a phone switched off every night had its morning walk spread over the days it was off, inventing days it had no reading for.
- **What a reboot actually costs** (Fase 24): every step walked since the last reading of that day, because nothing sampled them and the counter that held them was zeroed. With the phone left on nothing is lost — the next reading carries the whole interval. This is the one real hole in the passive path, and the only way out of it is to have a component that *is* allowed to record in the background do it: **Play services' Recording API** (`LocalRecordingClient`), on device, offline, accountless, across reboots. Health Connect cannot fill this role — it is a store, empty unless another app writes to it. Accepted cost, recorded in PLANNING Fase 24: the hours tsteps does not see are counted by Play services rather than by tsteps, and the feature does not exist on a de-Googled ROM (the app degrades to the sensor path and says so). It is **automatic and has no switch** (Fase 24c): a step counter that asks you to turn counting on is asking you to know what a subscription is. What the app owes the user is not a toggle but the truth about who counted, and `settings.config`'s `steps` section is exactly that and nothing else. Verified, not assumed: the merged manifest is permission-for-permission identical with and without the dependency — still no INTERNET, still no account, still no permanent service.
